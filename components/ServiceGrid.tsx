...
function ServiceIcon({ icon: Icon }: { icon: LucideIcon }) {
  return (
    <span className="-ml-2 flex h-14 w-14 shrink-0 -skew-x-12 items-center justify-center bg-brand-navy transition-colors duration-300 group-hover:bg-brand-aqua sm:-ml-3 sm:h-16 sm:w-16">
      <Icon
...
function ServiceCard({
  ...
  return (
    <article className="group relative flex min-h-[240px] flex-col overflow-hidden border-r border-b border-slate-200 bg-brand-white p-10 transition-colors duration-300 ease-out hover:bg-brand-mist sm:min-h-[260px] sm:p-12">
      <span
        className="absolute left-0 top-0 z-10 h-[4px] w-0 bg-brand-aqua transition-[width] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:w-full"
        aria-hidden="true"
      />
      <ServiceIcon icon={icon} />
      <div className="-ml-2 sm:-ml-3">
        <h3 className="font-display mt-10 text-xl font-bold uppercase leading-tight tracking-tight text-brand-slate sm:mt-11 sm:text-2xl">
          {title}
        </h3>
        <p className="mt-4 text-base leading-relaxed text-brand-navy sm:text-lg">
          {description}
        </p>
      </div>
    </article>
...
export function ServiceGrid() {
  return (
    <section id="services" className="bg-brand-mist py-20 sm:py-28">
      <SiteContainer>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="font-display text-3xl font-bold uppercase leading-[1.1] tracking-tighter sm:text-4xl lg:text-5xl">
            <span className="text-brand-slate">Our Complete </span>
            <span className="text-brand-aqua">Dispatch </span>
            <span className="text-brand-slate">Solution</span>
          </h2>
          <p className="mt-6 text-base leading-relaxed text-brand-navy sm:text-lg">
        ...
        <div className="mt-16 grid grid-cols-1 border-l border-t border-slate-200 md:grid-cols-2 lg:grid-cols-3">
          {services.map((service) => (
            <ServiceCard key={service.title} {...service} />
          ))}

          <article className="group relative flex min-h-[240px] flex-col justify-center overflow-hidden border-r border-b border-slate-200 bg-brand-navy p-10 text-white sm:min-h-[260px] sm:p-12 before:absolute before:inset-0 before:origin-bottom before:scale-y-0 before:bg-brand-aqua before:transition-transform before:duration-700 before:ease-[cubic-bezier(0.22,1,0.36,1)] before:content-[''] hover:before:scale-y-100">
